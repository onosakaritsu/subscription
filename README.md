...
- 多币种分别统计，不做自动汇率换算。
- 币种统一显示为“符号 / ISO 代码 / 中文名称”，例如 `¥ / CNY / 人民币`。
- 最近续费摘要优先显示已过期、今日续费、3 日内和 7 日内的启用订阅。
- JSON 导入/导出，导入前提供预览和差异摘要。
...
- 支持状态筛选、币种筛选和多种排序。
- 支持手动备份、备份下载和从外部备份 JSON 恢复，外部恢复前提供预览和差异摘要。
- 新增数据完整性检查，只读检查本地 JSON 的结构、日期、金额、币种和重复 id。
- 新增备份恢复演练提示，提醒定期下载备份并通过预览确认内容。
- 新增 ICS 续费日历导出，可将未来 12 个月续费日期导入系统日历。
...
Phase 5 将币种显示统一为“符号 / ISO 代码 / 中文名称”，例如 `¥ / CNY / 人民币`、`$ / USD / 美元`、`¥ / JPY / 日元`、`€ / EUR / 欧元`、`HK$ / HKD / 港币`。列表、统计卡片、最近续费摘要、续费日历和备份预览复用同一套金额格式。内部数据和 API 仍保存标准 ISO 代码，不做自动汇率换算。
...
可以点击“导出续费日历 ICS”下载 `subscriptions-renewals.ics`，将未来 12 个月内的启用订阅续费事件导入系统日历。该功能只导出文件，不做云同步或日历同步。

## 数据完整性检查

页面提供“数据完整性检查”卡片，可只读检查 `data/subscriptions.json` 是否存在空名称、负金额、非法日期、重复 id、未知币种等问题。检查不会自动修复或覆盖数据；未知币种会作为 warning 展示并保留原代码。
...
v1.0.0 发布前使用真实浏览器进行本地截图验收。截图保存在 `docs/screenshots/v1.0.0/`，不得包含真实个人订阅数据、临时浏览器缓存或第三方工具依赖。
...
本项目不构成法律、合规、财务、商业或专业建议。详细说明见 [DISCLAIMER.md](./DISCLAIMER.md)。隐私行为说明见 [PRIVACY.md](./PRIVACY.md)。
